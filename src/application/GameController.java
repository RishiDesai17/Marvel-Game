        ...
        public  void updateSwapUI(){ // needs to be called before each toggle true of swap menu.
        	...
        	if(index < player.party.size()) {
        		...
        		Avenger1Btn.setOnAction(e -> {
        			// if swapping away from hulk, restore base dmg on smash in case its been buffed
        			if(player.getStagedAvenger().name.equals("Hulk")) {
        ...
        public void updateAbilityUI() {
        	...
        	if(!player.canCancelSwap()) {
            	...
            	abilityBtn2.setOnAction(e -> {
            		...
            		// if hulk uses rage, need to update damage of hulk smash
            		if(stagedAvenger.getAbility(2).getName().equals("Rage")){
            		...
            		if(damageTaken == 0) {
            			dialogText.setText(stagedAvenger.name + " uses " + stagedAvenger.getAbility(2).getName() +".\nHulk is getting angry!! Hulk Smash now does " + stagedAvenger.getAbility(1).getBaseDamage() + " base damage!");
            		}
            		else {
            			dialogText.setText(stagedAvenger.name + " uses " + stagedAvenger.getAbility(2).getName() +"!!\nABILITY BASE DAMAGE : " + String.format("%.0f",baseDamage) +
            					(bonus >= 0 ? ("\nMATCH-UP ABILITY BONUS DAMAGE: " + String.format("%.0f",bonus)) : 
...
//        	count += 1;
	 ...
	 private void setFxml(Player p1,Player p2){
		...
		public void playGameScreenMedia() {
			Media media = null;
			try {
				media = new Media(new File("src/application/media/Avengers Endgame (Avengers Assemble) Final Fight Soundtrack.mp3").toURI().toURL().toString());
